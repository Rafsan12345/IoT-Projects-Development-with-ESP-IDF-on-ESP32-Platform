...
void app_main(void)
{
    ...
    FILE *f = fopen("/spiflash/hello.txt", "a"); //a  = paaend w = write by previous data replace
    fprintf(f, "Written using ESP-IDF\n");
    fprintf(f, "UIU EEE ESP-IDF\n");
    fclose(f);
    ...
    //
    char line2[128];  // Buffer to store each line
	while (fgets(line2, sizeof(line2), f)) {
	    ESP_LOGI(TAG, "%s", line2);  // Print each line
	}

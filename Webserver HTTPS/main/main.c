...
// PEM সার্টিফিকেট প্রিন্ট করার জন্য
...
static httpd_handle_t start_webserver(void)
{
    ...
    ESP_LOGI(TAG, "CACERT Start: %p, End: %p", cacert_pem_start, cacert_pem_end);
ESP_LOGI(TAG, "PRVTKEY Start: %p, End: %p", prvtkey_pem_start, prvtkey_pem_end);

    
    printf("CACERT Start: %p, End: %p\n", cacert_pem_start, cacert_pem_end);
printf("PRVTKEY Start: %p, End: %p\n", prvtkey_pem_start, prvtkey_pem_end);
    ...
    if (ESP_OK != ret)
    {
        ESP_LOGI(TAG, "Error starting server! 77");
        return NULL;
    }
